...
program
  ...
  .requiredOption(
    "-a, --asset <asset>",
    "Name of asset we are claiming rewards for (either WBTC or ETH)"
  )
  .requiredOption(
    "-b, --blockNBR <blockNBR>",
    "Starting block number to start claiming from"
  );
...
async function claimRewards() {
  ...
  const volatilityContract = new web3.eth.Contract(
    volatilityJSON.abi,
    deployments[network].RibbonVolatilityLogic
  );

  const rewardsAddress =
    program.asset == "ETH"
      ? "0x7c83Ed5eeC3370CcC98FC43ce871c7416bD7B803"
      : "0xBae7BE4e4a5c376950B8DB86D9D0DD1BaFc7C318";
  const hegicOptionsAddress =
    program.asset == "ETH"
  ...
  var optionIDs = await getOptionIDs(
    hegicOptionsAddress,
...
async function getOptionIDs(
  ...
  const optionsContract = new web3.eth.Contract(
    hegicOptionsJSON.abi,
    optionsAddress
  );

  var filter = { account: proxyContractAddress };
  var optionIDs = (
    await optionsContract.getPastEvents("Create", {
      filter,
      fromBlock: startBlockNBR,
      toBlock: "latest",
    })
  ).map((a) => new BN(a["returnValues"]["0"]));
  return optionIDs;
}
