  ...
  describe(`${params.name}`, () => {
    ...
    describe("#depositYearnToken", () => {
      ...
      it("deposits successfully", async function () {
        ...
        const depositAmountInAsset = wmul(depositAmount, pricePerShare);

        await this.assetContract
          .connect(userSigner)
          .approve(this.vault.address, depositAmount);

        const res = await this.vault.depositYearnToken(depositAmount);
        const receipt = await res.wait();
      ...
      it("returns the correct number of shares back", async function () {
        ...
        const depositAmountInAsset = wdiv(depositAmount, pricePerShare);

        // first user gets 3 shares
        await this.vault
          .connect(userSigner)
          .depositYearnToken(depositAmountInAsset);
      ...
      it("reverts when no value passed", async function () {
        await expect(
          this.vault.connect(userSigner).depositYearnToken(0)
        ).to.be.revertedWith(/Insufficient asset balance/);
      });
      ...
      it("does not inflate the share tokens on initialization", async function () {
        ...
        await this.vault
          .connect(userSigner)
          .depositYearnToken(BigNumber.from("10000000000"));
      ...
      it("closes the short after expiry", async function () {
        ...
        await expect(closeTx)
          ...
          .withArgs(
            ...
            wdiv(wmul(this.depositAmount, LOCKED_RATIO), pricePerShare).sub(1),
            user
          );
      });
    });
    ...
    describe("#rollToNextOption", () => {
      ...
      it("reverts when calling before expiry", async function () {
        ...
        const withdrawBuffer = wmul(this.depositAmount, WITHDRAWAL_BUFFER);
    ...
    if (params.depositAsset === WETH_ADDRESS) {
      describe("#withdrawETH", () => {
        ...
        it("should withdraw funds, sending withdrawal fee to feeRecipient if <10% (deposit is yield token)", async function () {
          ...
          await this.vault.depositYearnToken(
            wdiv(parseEther("1"), pricePerShare)
          );
    ...
    describe("#withdraw", () => {
      ...
      it("should withdraw funds, sending withdrawal fee to feeRecipient (deposit is yield token)", async function () {
        ...
        await this.vault.depositYearnToken(depositAmount);
